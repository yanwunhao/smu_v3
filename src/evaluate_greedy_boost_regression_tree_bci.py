...
# classifier = XGBClassifier(n_estimators=50, base_score=0.5, booster='gblinear', eta=0.3)
# classifier = XGBClassifier(n_estimators=50, base_score=0.5, booster='dart', eta=0.3)
# classifier = RandomForestClassifier(n_estimators=50, max_depth=3, min_samples_split=10, random_state=10)
classifier = LogisticRegression(random_state=0)

cv = KFold(n_splits=5, shuffle=True, random_state=100)
ax = plot_learning_curve(classifier, "logistic-bci", normalized_feature_matrix, target, ax=None, cv=cv)
plt.show()