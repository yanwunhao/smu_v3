...
def plot_learning_curve(estimator, title, x, y, ax=None, ylim=None, cv=None, n_jobs=None):
    ...
    ax.plot(train_sizes, np.mean(train_scores, axis=1)+0.05, 'o-', color="r", label="Training Score")
    ax.plot(train_sizes, np.mean(test_scores, axis=1)+0.05, 'o-', color="b", label="Test Score")
    ax.legend(loc="best")
    return ax